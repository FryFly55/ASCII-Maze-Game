...
int main()
{
    ...
    while(seconds <= titleScreenDelay) {
        const char* text = "MAZE GAME";
        int row = 15;
        ...
        const char* credits = "made by FryFly";
        row = 17;
        index = (consoleWidth - strlen(credits)) / 2;
        for (int i = 0; i < strlen(credits); i++) {
            screen[(row * consoleWidth) + index + i] = credits[i];
        }
